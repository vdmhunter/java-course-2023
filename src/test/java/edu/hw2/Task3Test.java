...
import edu.hw2.task3.randomgenerators.FailingRandomGenerator;
...
class Task3Test {
    ...
    void popularCommandExecutor_TestUpdatePackagesWithFailedFaultyConnection() {
        ...
        int maxAttempts = 4;
        var expectedType = ConnectionException.class;
        Executable executable = () -> new PopularCommandExecutor(connectionManager, maxAttempts).updatePackages();
    ...
    @Test
    @DisplayName("Test PopularCommandExecutor throws IllegalStateException with FaultyConnection")
    void popularCommandExecutor_TestUpdatePackagesThrowsIllegalStateException() {
        // Arrange
        var randomGenerator = new FailingRandomGenerator();
        var connectionManager = new FaultyConnectionManager(randomGenerator);
        int maxAttempts = 1;
        var expectedType = IllegalStateException.class;
        Executable executable = () -> new PopularCommandExecutor(connectionManager, maxAttempts).updatePackages();
